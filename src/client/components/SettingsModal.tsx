    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto bg-card border-border/80">
        ...
        <form onSubmit={handleSave} className="space-y-4 py-2">
          ...
          {/* Tailscale 24/7 Remote URL Banner */}
          {status?.tailscaleIp && (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(`http://${status.tailscaleIp}:${status.port}`);
                    setCopiedTailscale(true);
                    setTimeout(() => setCopiedTailscale(false), 2000);
                  }}
                  className="text-[11px] flex items-center gap-1 text-emerald-400 hover:underline"
                >
                  {copiedTailscale ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                  {copiedTailscale ? 'Copied' : 'Copy'}
                </button>
              </div>
              <div className="font-mono text-xs text-foreground bg-background/80 rounded-md p-2 border border-border/50 break-all select-all">
                http://{status.tailscaleIp}:{status.port}
              </div>
              <p className="text-[11px] text-muted-foreground">
                Encrypted remote access from anywhere in the world on cellular data or remote Wi-Fi.
              </p>
            </div>
          )}
